...
The 3 different chips that makes up this design is the \gls{sampa}, the \gls{gbt}, and the \gls{cru}.
Out of the 3, the \gls{sampa} and the \gls{cru} are the most interesting as they are still being developed and testing them can give a lot of valuable feedback.
...
The signals will be processed, compressed, digitized, and temporarily stored in the \gls{sampas} memory before it is passed along.
The \gls{sampa} has 32 integrated channels, which separately and asynchronously process the analog signals coming from the detector.
Each channel has a readout speed of 10 bit on a 10 MHz clock, which combined results in 400 Mb/s.
...
Each e-link has as said in the previous section a speed of 320 Mb/s, which sums up to 1.28 Mb/s.
