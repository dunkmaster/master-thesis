...
\paragraph{•}%OBS! Reference book here.
...
\paragraph{•}
\begin{minipage}{\linewidth}
\lstinputlisting[caption=Calculating the space between two peaks in a timeframe.,firstline=64,lastline=70,label=lst:calc-space]{codelistings/dg.cpp}
\end{minipage}
...
In \ref{fig:occ-28} the randomly picked normal distribution used for this simulation is shown.
...
\paragraph{•}
As seen in \ref{fig:28-occ} the channel buffers fill up over time, and there is to much data for the serial outs to read.
...
\paragraph{•}
Since 28 percent mean occupancy was to much for the model to handle, the next simulation will try five percent less, i.e 23.
...
This is a number which is more then acceptable, and the simulation could most likely run forever and it would not matter.
...
\paragraph{•}
When looking at 26 percent mean occupancy it can already be established that this is to much.
...
25 percent was exactly where the simulation model started to have trouble reading the data quickly enough, confirming that the results found where indeed valid.
...
Since the data is collected from RUN 1, they have slightly less occupancy then what is expected for RUN 3, but the it still has the most realistic shape available.
...
\paragraph{•}
One of the reasons for creating pileup data is to increase the amount of real samples, which possible can give more interesting results then using plain black event data.
...
\paragraph{•}
The pileup results showed a definite decrease in the compression factor.
Reasons being that now that samples have been piled on top of each other, the difference between neighbouring samples is bigger, which results in a worse Huffman table.
Even so, the decrease was more then expected, only getting an average compression factor of three.
The factor varies between 2.6 and 3.5, which is still decent enough for the model to handle.
The compressed size of each time frame is now on average 100 words more then the normal black events.
...
However there are multiple things to take away from these results.
For all of the scenarios, the buffers never came close to being full.
It seems that for the black event dataset, the Huffman is able to perform better then \gls{zs}, but only by a small margin.
Huffman had a decrease in performance then \gls{zs} when comparing normal black events, and pileup.
...
\paragraph{•}
There was no way to see how the model behaved when Huffman didn't compress good enough, this is hopefully something that will be seen when trying the synthetic data made for RUN 3.


\subsection{Synthetic Data for RUN 3}
\subsubsection{Preface}
\paragraph{•}
Lorem ipsum dolor sit amet, consectetur adipiscing elit. Etiam dolor dui, scelerisque et urna quis, volutpat dapibus massa. Vivamus consequat ante eu metus lobortis vestibulum. Cras laoreet nisl quam, et rhoncus quam efficitur ac. Etiam at suscipit felis. Maecenas aliquet nibh sit amet ultrices placerat. Pellentesque lobortis ultricies nunc, consectetur pharetra leo sollicitudin id. Maecenas varius rhoncus posuere. Sed euismod molestie ante eu malesuada. Ut sem eros, viverra ut iaculis a, cursus id libero. Proin egestas pulvinar volutpat. Morbi ac eleifend lectus, in suscipit augue.
