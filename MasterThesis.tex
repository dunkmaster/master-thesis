...
\end{table}
\paragraph{•}
Looking at the deceleration of the \gls{sampa} module it seems that, minus the data compression, everything is structured as the chip itself.
It stores every channels header and data buffer in an array, 

Each of the channels will contain a lot of functionality when this is translated into \gls{sc}, so to avoid duplication of code, each channel will be its own \gls{sc} module.
This means that every \gls{sampa} will consist of an array of \codeword{Channel} modules, all running there separate asynchronous thread.
